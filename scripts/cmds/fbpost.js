...
module.exports = {
  ...
  onReply: async function ({ Reply, event, api, commandName }) {
    ...
    async function uploadAttachments(attachments, botID) {
      const uploads = [];
      ...
      for (let i = 0; i < attachments.length; i++) {
        ...
        if (attachment.path) {
          form.append("file", fs.createReadStream(attachment.path));
        } else {
    ...
    if (type == "whoSee") {
      ...
      if (event.body != "0") {
        ...
        for (const attach of attachments) {
          if (attach.type === "photo") {
            ...
            const imgPath = __dirname + `/cache/imagePost_${Date.now()}.png`;
            fs.writeFileSync(imgPath, Buffer.from(getFile));
            allStreamFile.push({ path: imgPath });
          } else if (attach.type === "video") {
            const videoFile = await axios.get(attach.url, { responseType: "stream" });
            const videoPath = __dirname + `/cache/videoPost_${Date.now()}.mp4`;
            videoFile.data.pipe(fs.createWriteStream(videoPath));
            await new Promise(resolve => videoFile.data.on("end", resolve));
            allStreamFile.push({ path: videoPath });
          }
        }
        const uploadFiles = await uploadAttachments(allStreamFile, botID);
      ...
      api.httpPost('https://www.facebook.com/api/graphql/', form, (e, info) => {
        ...
        try {
          ...
          try {
            fs.readdirSync(__dirname + "/cache").forEach(file => fs.unlinkSync(__dirname + "/cache/" + file));
          } catch (e) {}
